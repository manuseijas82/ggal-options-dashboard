...
def get_opciones_byma(spot):
    ...
    @staticmethod
    def _cadena_sintetica(spot, iv_base=0.65, dias=38):
        r = 0.05
        T = dias / 365
        rows = []
        strikes = [round(spot * m, -1) for m in
                   [0.80, 0.85, 0.875, 0.90, 0.925,
                    0.95, 0.975, 1.0, 1.025, 1.05,
                    1.075, 1.10, 1.125, 1.15, 1.20]]
        vto_str = (datetime.today() + timedelta(days=dias)).strftime('%d/%m/%Y')
        for k in strikes:
            iv_skew = iv_base + 0.10 * ((spot - k) / spot)
            ...
            for flag, precio in [('call', pc), ('put', pp)]:
                if precio > 0.5:
                    rows.append({
                        ...
                        'vol_op': int(np.random.randint(50, 500)),
                        'oi': int(np.random.randint(100, 2000)),
                        'dias': dias,
                        'vto': vto_str,
                    })
        return rows
...
class AlertEngine:
    ...
    def _pares(self, flag):
        return self.df[self.df['tipo'] == flag].sort_values('strike')
                ...
                for j in range(i + 1, len(strikes)):
                    ...
                    if ratio >= 1.85:
                        ...
                        alertas.append({
                            ...
                            'bases': f'{flag.upper()} {k1:.1f} / {flag.upper()} {k2:.1f}',
                            'senal': f'Percentil: {min(pct, 99):.1f}% (Señal CARA → Vendé)',
                            'senal_color': '#f85149',
                            'precio': round(abs(credito), 2),
                            'spot': self.spot,
                            'detalle': (
                                f"👉 Venta 1 {flag.upper()} {k1:.1f} a ${p1:.2f} / "
                                f"Compra 2 {flag.upper()} {k2:.1f} a ${p2:.2f} | "
                ...
                for j in range(i + 1, len(strikes)):
                    ...
                    if ratio <= 0.55:
                        ...
                        alertas.append({
                            ...
                            'bases': f'{flag.upper()} {k1:.1f} / {flag.upper()} {k2:.1f}',
                            'senal': f'OTM sobre-valuada | Ratio: {ratio:.2f}x | Percentil: {min(pct, 99):.1f}%',
                            'senal_color': '#2ea043',
                            'precio': round(abs(credito), 2),
                            'spot': self.spot,
                            'detalle': (
                                f"👉 Compra 1 {flag.upper()} {k1:.1f} a ${p1:.2f} / "
                                f"Venta 2 {flag.upper()} {k2:.1f} a ${p2:.2f} | "
                                f"{'✅ Crédito' if credito >= 0 else '⚠️ Débito'}: ${abs(credito):.2f} | "
                                f"Ratio: {ratio:.2f}x | Δ strikes: {dk:.1f}%"
                            ),
        ...
        for i in range(len(strikes) - 1):
            ...
            if rr >= 1.2:
                alertas.append({
                    ...
                    'bases': f'CALL {k1:.1f} / CALL {k2:.1f}',
                    'senal': f'R/R: {rr:.1f}x | Δ strikes: {dk:.1f}%',
                    'senal_color': '#2ea043',
                    'precio': round(debito, 2),
                    'spot': self.spot,
                    'detalle': (
                        f"👉 Compra 1 CALL {k1:.1f} a ${p1:.2f} / "
                        f"Venta 1 CALL {k2:.1f} a ${p2:.2f} | "
                        f"Débito: ${debito:.2f} | Máx ganancia: ${max_gan:.2f} | "
                        f"R/R: {rr:.1f}x | BE: ${k1 + debito:.1f}"
                    ),
        ...
        for i in range(len(strikes) - 1):
            ...
            if rr >= 1.2:
                alertas.append({
                    ...
                    'bases': f'PUT {k1:.1f} / PUT {k2:.1f}',
                    'senal': f'R/R: {rr:.1f}x | Δ strikes: {dk:.1f}%',
                    'senal_color': '#f85149',
                    'precio': round(debito, 2),
                    'spot': self.spot,
                    'detalle': (
                        f"👉 Compra 1 PUT {k2:.1f} a ${p2:.2f} / "
                        f"Venta 1 PUT {k1:.1f} a ${p1:.2f} | "
                        f"Débito: ${debito:.2f} | Máx ganancia: ${max_gan:.2f} | "
                        f"R/R: {rr:.1f}x | BE: ${k2 - debito:.1f}"
                    ),
                    'percentil': min(rr * 30, 99),
                    'timestamp': datetime.now().strftime('%H:%M'),
                })
        ...
        for _, crow in calls.iterrows():
            ...
            if not same.empty:
                ...
                alertas.append({
                    ...
                    'card_class': cc,
                    'titulo': f'⚖️ STRADDLE {tag} | Strike {kc:.1f}',
                    'estrategia': f'STRADDLE {tag}',
                    'bases': f'CALL {kc:.1f} / PUT {kc:.1f}',
                    'senal': f'IV avg: {iv_avg * 100:.1f}% | Percentil: {pct:.1f}%',
                    'senal_color': sc,
                    'precio': round(total, 2),
                    'spot': self.spot,
                    'detalle': (
                        f"{'Venta' if tag == 'VENDIDO' else 'Compra'} 1 CALL {kc:.1f} a ${pc:.2f} + "
                        f"{'Venta' if tag == 'VENDIDO' else 'Compra'} 1 PUT {kc:.1f} a ${pp:.2f} | "
                        f"Prima {'cobrada' if tag == 'VENDIDO' else 'pagada'}: ${total:.2f} | "
                        f"Rango neutro: ±{be_pct:.1f}% | "
                        f"BE: ${kc - total:.1f} / ${kc + total:.1f}"
                    ),
                    'percentil': pct,
                    'timestamp': datetime.now().strftime('%H:%M'),
                })
            ...
            for _, prow in puts.iterrows():
                ...
                alertas.append({
                    ...
                    'detalle': (
                        f"{'Venta' if tag == 'VENDIDO' else 'Compra'} 1 CALL {kc:.1f} a ${pc:.2f} + "
                        f"{'Venta' if tag == 'VENDIDO' else 'Compra'} 1 PUT {kp:.1f} a ${pp:.2f} | "
                        f"Prima {'cobrada' if tag == 'VENDIDO' else 'pagada'}: ${total:.2f} | "
                        f"Rango: ${kp - total:.1f} / ${kc + total:.1f}"
                    ),
                    'percentil': pct,
                    'timestamp': datetime.now().strftime('%H:%M'),
                })
        ...
        for _, row in self.df.iterrows():
            ...
            if pct >= 88:
                alertas.append({
                    ...
                    'titulo': f'🔥 IV EXTREMA — {row["tipo"].upper()} {row["strike"]:.1f}',
                    'estrategia': 'VENTA DE VOLATILIDAD',
                    'bases': f'{row["tipo"].upper()} {row["strike"]:.1f}',
                    'senal': f'IV: {iv * 100:.1f}% | Percentil: {pct:.1f}%',
                    'senal_color': '#f85149',
                    'precio': round(row['precio'], 2),
                    'spot': self.spot,
                    'detalle': (
                        f"👉 Venta 1 {row['tipo'].upper()} {row['strike']:.1f} "
                        f"a ${row['precio']:.2f} | IV extrema: {iv * 100:.1f}% | "
                ...
                alertas.append({
                    ...
                    'titulo': f'💎 IV MÍNIMA — {row["tipo"].upper()} {row["strike"]:.1f}',
                    ...
                    'spot': self.spot,
                    'detalle': (
                        f"👉 Compra 1 {row['tipo'].upper()} {row['strike']:.1f} "
                        f"a ${row['precio']:.2f} | IV baja: {iv * 100:.1f}% | "
...
def render_alert_card(a):
    ...
    <div class="alert-card {a['card_class']}">
      ...
      🎯 <b>Señal:</b> <span style="color:{a['senal_color']};font-weight:700">{a['senal']}</span><br>
      💰 <b>Precio armado:</b> ${a['precio']:.2f}<br>
      📈 <b>Spot GGAL:</b> ${a['spot']:,.2f}
      {iv_str}
      <div class="pct-bar" style="width:{bar_w}%;background:{bar_c};"></div>
      <div class="alert-detail">🔍 <b>Detalle:</b> {a['detalle']}</div>
...
def render_cadena_table(df, spot):
    if df.empty:
...
def render_iv_smile(df, spot):
    ...
    fig.update_layout(
        title='📊 Curva de Volatilidad Implícita',
    ...
    try:
        ...
        if 'BULL CALL' in strat:
            ...
            payoff = sign * (np.maximum(S_range - kc, 0) + np.maximum(kp - S_range, 0)) + \
                     (precio if sign == -1 else -precio)
        elif 'RATIO INVERTIDO' in strat:
            k1, k2 = ks[0], ks[1]
            flag = 'C' if 'CALL' in bases else 'P'
            if flag == 'C':
                payoff = -np.maximum(S_range - k1, 0) + 2 * np.maximum(S_range - k2, 0) + precio
            else:
...
def main():
    ...
    with st.spinner("📡 Obteniendo datos de mercado..."):
        ...
        df = GGALDataFetcher.get_opciones_byma(spot or 6400)

    if spot is None:
        st.warning("⚠️ Sin precio spot en vivo.")
        spot = st.number_input("Precio spot manual:",
                               min_value=100.0, max_value=99999.0,
    ...
    with col1:
        render_metric("💵 Spot GGAL", f"${spot:,.2f}", '#58a6ff')
    with col2:
        render_metric("📋 Opciones", str(len(df)) if not df.empty else "—", '#d29922')
    with col3:
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "🎯 Alertas", "📊 IV Smile", "📋 Cadena de Opciones", "📉 Payoff"
    ])
    ...
    if not df.empty:
        ...
        st.error("❌ Sin datos de opciones.")
    ...
    with tab1:
        if not alertas:
            st.info("🔍 Sin alertas. Bajá el percentil mínimo o esperá horario de mercado.")
        else:
    ...
    with tab2:
        if not df.empty and 'iv' in df.columns:
            ...
            st.info("Sin datos de IV.")

    with tab3:
        st.markdown("#### 📋 Cadena de Opciones GGAL")
        if not df.empty:
            tipo_filter = st.radio("Filtrar:", ['Todas', 'call', 'put'], horizontal=True)
    ...
    with tab4:
        if not alertas:
            st.info("Generá alertas primero.")
        else:
            opciones_payoff = {f"{a['estrategia']} | {a['bases']}": a for a in alertas}
            sel = st.selectbox("Seleccioná estrategia:", list(opciones_payoff.keys()))
            if sel:
                render_alert_card(opciones_payoff[sel])
                render_payoff(opciones_payoff[sel], spot)
